...
- [x] **HIER-01**: Developer can declare child agents via `child { }` in the `agent { }` DSL — child runs as a tool call (spawn model) and its result feeds back into the parent loop as a `ToolResult`
