...
class ClaudeBackendTest {
    ...
    @Test
    fun `emits Text chunk for text content block`() =
        runTest {
            val textBlock =
                mockk<TextBlock> {
    ...
    @Test
    fun `emits ToolCall chunk for tool_use content block`() =
        runTest {
            val toolUseBlock =
                mockk<ToolUseBlock> {
                    every { id() } returns "call_abc123"
                    every { name() } returns "search"
                    every { _input() } returns
                        com.anthropic.core.JsonValue
                            .from(mapOf("query" to "kotlin coroutines"))
                }
            val contentBlock =
                mockk<ContentBlock> {
    ...
    @Test
    fun `system messages are extracted and not included in message list`() =
        runTest {
            val textBlock =
                mockk<TextBlock> {
