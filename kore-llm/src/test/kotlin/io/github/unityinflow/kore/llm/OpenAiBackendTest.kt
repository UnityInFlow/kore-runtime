package io.github.unityinflow.kore.llm
...
import io.github.unityinflow.kore.core.ConversationMessage
...
class OpenAiBackendTest {
    ...
    @Test
    fun `emits Text chunk for text content`() =
        runTest {
            val message =
                mockk<ChatCompletionMessage> {
    ...
    @Test
    fun `emits ToolCall chunk for tool calls in response`() =
        runTest {
            val functionCall =
                mockk<ChatCompletionMessageFunctionToolCall.Function> {
                    every { name() } returns "calculator"
                    every { arguments() } returns """{"expression": "2+2"}"""
                }
            val functionToolCall =
                mockk<ChatCompletionMessageFunctionToolCall> {
                    every { id() } returns "call_xyz789"
                    every { function() } returns functionCall
                }
            val toolCallWrapper =
                mockk<ChatCompletionMessageToolCall> {
    ...
    @Test
    fun `emits Usage chunk when usage is present`() =
        runTest {
            val message =
                mockk<ChatCompletionMessage> {
    ...
    @Test
    fun `system messages are converted to system role params`() =
        runTest {
            val message =
                mockk<ChatCompletionMessage> {
